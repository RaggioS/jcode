...
pub fn import_session_from_file(path: &Path, session_id: &str) -> Result<Session> {
    ...
    let imported_len = collected.len() + usize::from(recap.is_some());
    if crate::session::session_exists(&jcode_session_id)
        && let Ok(existing) = Session::load(&jcode_session_id)
        && existing.messages.len() > imported_len
    {
        return Ok(existing);
    }

    // Create the jcode session. Tag it with the configured default provider and
    // model so resume continues with the LOCAL model: the transcript records
    // Claude's model, and `restore_session` would otherwise try to switch to it
    // (unavailable on the offline lane).
